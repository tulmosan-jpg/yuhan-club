import 'package:flutter/widgets.dart';

import '../models/activity.dart';
import '../models/report.dart';

/// 활동 종류의 현지화 라벨.
String activityTypeLabel(BuildContext context, ActivityType type) =>
    tr(context, 'atype_${type.name}');

/// 보고서 역할의 현지화 라벨.
String reportRoleLabel(BuildContext context, ReportRole role) =>
    tr(context, 'role_${role.name}');

/// 보고서 상태의 현지화 라벨.
String reportStatusLabel(BuildContext context, ReportStatus status) =>
    tr(context, 'status_${status.name}');

/// 매우 가벼운 앱 내 다국어 사전. 키 → [한국어, 영어].
/// 위젯에서 `tr(context, 'key')` 로 현재 로케일 문자열을 얻는다.
/// 동적 값은 `tr(context, 'key', {'name': '홍길동'})` 처럼 `{name}` 치환.
const Map<String, List<String>> _dict = {
  // 공통 / 앱
  'app_title': ['유한대 식품영양학과', 'Yuhan Food & Nutrition'],
  'dept_full': ['유한대학교 식품영양학과', 'Yuhan University Food & Nutrition'],

  // 하단 네비게이션
  'nav_home': ['홈', 'Home'],
  'nav_reports': ['보고서', 'Reports'],
  'nav_activities': ['대외활동', 'Activities'],
  'nav_attendance': ['출석', 'Attendance'],
  'nav_certs': ['자격증', 'Certs'],
  'nav_rewards': ['리워드', 'Rewards'],

  // 로그인
  'login_subtitle': ['유한대 식품영양학과 앱에 로그인', 'Sign in to the department app'],
  'signup_subtitle': ['새 계정 만들기', 'Create a new account'],
  'admin_login_subtitle': ['운영진 전용 관리자 로그인', 'Staff-only admin login'],
  'login_user': ['일반 로그인', 'User'],
  'login_admin': ['관리자 로그인', 'Admin'],
  'admin_login': ['관리자 로그인', 'Admin login'],
  'not_admin': ['관리자 계정이 아닙니다.', 'This is not an admin account.'],
  'use_admin_login': ['관리자 계정입니다. 관리자 로그인을 이용하세요.', 'This is an admin account. Please use admin login.'],
  'admin_check_failed': ['권한 확인에 실패했어요. 네트워크를 확인해주세요.', 'Could not verify your permissions. Please check your network.'],
  'retry': ['다시 시도', 'Retry'],
  'stock_saved': ['재고를 변경했어요.', 'Stock updated.'],
  'stock_save_failed': ['재고 변경 실패', 'Failed to update stock'],
  // 보고서 활동일 제한
  'report_no_activity_days': ['멘토가 등록한 활동일이 없어요. 활동일이 등록된 뒤에 작성할 수 있어요.',
      'No activity days registered yet. Reports can be written once your mentor registers activity days.'],
  'report_date_not_activity': ['활동일에만 보고서를 작성할 수 있어요. 날짜를 다시 선택해주세요.',
      'Reports can only be written for registered activity days. Please pick a valid date.'],
  // 리워드 수령 확인(서명+영수증)
  'redeem_confirm_title': ['수령 확인', 'Pickup confirmation'],
  'redeem_sign_section': ['수령자 서명', 'Recipient signature'],
  'redeem_sign_hint': ['여기에 서명해주세요', 'Sign here'],
  'redeem_sign_clear': ['지우기', 'Clear'],
  'redeem_receipt_section': ['주문서/영수증 사진', 'Order slip / receipt photo'],
  'redeem_receipt_camera': ['촬영', 'Camera'],
  'redeem_receipt_gallery': ['앨범', 'Gallery'],
  'redeem_complete_button': ['수령 완료', 'Complete pickup'],
  'redeem_proof_required': ['서명과 사진을 모두 등록해야 완료할 수 있어요.',
      'Both signature and photo are required to complete.'],
  // 수령자 명단 엑셀
  'roster_export': ['수령자 명단 내보내기(엑셀)', 'Export roster (Excel)'],
  'roster_empty': ['수령 완료된 쿠폰이 아직 없어요.', 'No redeemed coupons yet.'],
  'roster_export_failed': ['명단 내보내기 실패', 'Failed to export roster'],
  // 학과 가입코드
  'join_code': ['학과 가입코드', 'Department join code'],
  'err_join_code': ['가입코드를 입력해주세요.', 'Please enter the join code.'],
  'join_code_wrong': ['가입코드가 올바르지 않아요. 학과에서 받은 코드를 확인해주세요.',
      'Invalid join code. Please check the code from your department.'],
  'join_code_not_set': ['가입코드가 아직 설정되지 않았어요. 관리자에게 문의해주세요.',
      'Join code is not set yet. Please contact an administrator.'],
  'join_code_section': ['학과 가입코드', 'Department join code'],
  'join_code_hint': ['회원가입 시 이 코드를 입력해야 가입할 수 있어요. 학과 구성원에게만 공유하세요.',
      'New members must enter this code to sign up. Share it only within the department.'],
  'join_code_saved': ['가입코드를 저장했어요.', 'Join code saved.'],
  'join_code_verify_title': ['가입코드 인증', 'Verify join code'],
  'join_code_verify_body': ['리워드는 학과 구성원만 이용할 수 있어요. 학과에서 받은 가입코드를 입력해주세요.',
      'Rewards are for department members only. Enter the join code from your department.'],
  'join_code_verified': ['인증됐어요! 이제 리워드를 이용할 수 있어요.', 'Verified! You can now use rewards.'],
  'join_code_too_many': ['시도가 너무 많았어요. 30분 후 다시 시도해주세요.', 'Too many attempts. Try again in 30 minutes.'],
  'redeem_error': ['사용 처리에 실패했어요. 네트워크를 확인하고 다시 시도해주세요.',
      'Failed to redeem. Check your network and try again.'],
  'reward_load_failed': ['리워드 정보를 불러오지 못했어요. 아래로 당겨 새로고침해주세요.',
      'Failed to load rewards. Pull down to refresh.'],
  'save_failed_perm': ['저장할 수 없어요. 입력 내용이 규정(길이 제한 등)을 벗어났거나 권한이 없어요.',
      'Could not save. The content violates limits or you lack permission.'],
  'reward_needs_join_code': ['가입코드 인증이 필요해요. 관리자에게 문의해주세요.',
      'Join-code verification required. Please contact an administrator.'],
  // 비밀번호 변경/임시 비밀번호
  'pw_change_title': ['비밀번호 변경', 'Change password'],
  'pw_change_desc': ['현재(또는 임시) 비밀번호를 확인한 뒤 새 비밀번호로 바꿔요.',
      'Verify your current (or temporary) password, then set a new one.'],
  'pw_current': ['현재 비밀번호', 'Current password'],
  'pw_new': ['새 비밀번호 (6자 이상)', 'New password (6+ chars)'],
  'pw_change_confirm': ['변경', 'Change'],
  'pw_current_empty': ['현재 비밀번호를 입력해주세요.', 'Please enter your current password.'],
  'pw_change_done': ['비밀번호를 변경했어요.', 'Password changed.'],
  'pw_change_now': ['지금 변경', 'Change now'],
  'later': ['나중에', 'Later'],
  'pw_temp_title': ['새 비밀번호를 설정해주세요', 'Set a new password'],
  'pw_temp_body': ['임시 비밀번호로 로그인했어요.\n계속 사용하기 전에 나만 아는 비밀번호로 바꿔주세요.',
      'You signed in with a temporary password.\nPlease change it to your own before continuing.'],
  'pw_reset_sent_title': ['재설정 메일을 보냈어요', 'Reset email sent'],
  'pw_reset_sent_body': ['{email} 로 비밀번호 재설정 링크를 보냈어요.\n\n몇 분 안에 오지 않으면 스팸함을 확인해주세요. 메일이 계속 오지 않으면 멘토(관리자)에게 임시 비밀번호 발급을 요청할 수 있어요.',
      'A password-reset link was sent to {email}.\n\nIf it does not arrive within a few minutes, check your spam folder. If it still does not arrive, ask your mentor (admin) to issue a temporary password.'],
  'update_title': ['새 버전이 있어요', 'Update available'],
  'update_body': ['더 나은 사용을 위해 최신 버전으로 업데이트해주세요.', 'Please update to the latest version for the best experience.'],
  'update_now': ['업데이트', 'Update'],
  'update_later': ['나중에', 'Later'],
  'login': ['로그인', 'Log in'],
  'signup': ['회원가입', 'Sign up'],
  'signup_done': ['가입되었습니다! 환영해요 🎉', 'Signed up! Welcome 🎉'],
  'name': ['이름', 'Name'],
  'email': ['이메일', 'Email'],
  'password': ['비밀번호', 'Password'],
  'err_name': ['이름을 입력해주세요.', 'Please enter your name.'],
  'err_email_empty': ['이메일을 입력해주세요.', 'Please enter your email.'],
  'err_email_invalid': ['올바른 이메일을 입력해주세요.', 'Please enter a valid email.'],
  'err_pw_len': ['비밀번호는 6자 이상이어야 합니다.', 'Password must be at least 6 characters.'],
  'remember_email': ['아이디 저장', 'Remember email'],
  'auto_login': ['자동 로그인', 'Auto login'],
  'forgot_pw': ['비밀번호 찾기', 'Forgot password'],
  'have_account': ['이미 계정이 있으신가요?', 'Already have an account?'],
  'no_account': ['계정이 없으신가요?', "Don't have an account?"],
  'enter_email_first': ['이메일을 먼저 입력해주세요.', 'Please enter your email first.'],
  'pw_reset_sent': ['비밀번호 재설정 메일을 보냈습니다.', 'Password reset email sent.'],

  // 인증 에러
  'auth_invalid_email': ['이메일 형식이 올바르지 않습니다.', 'The email format is invalid.'],
  'auth_user_disabled': ['비활성화된 계정입니다.', 'This account has been disabled.'],
  'auth_bad_cred': ['이메일 또는 비밀번호가 올바르지 않습니다.', 'Incorrect email or password.'],
  'auth_email_in_use': ['이미 가입된 이메일입니다.', 'This email is already registered.'],
  'auth_weak_pw': ['비밀번호는 6자 이상이어야 합니다.', 'Password must be at least 6 characters.'],
  'auth_network': ['네트워크 연결을 확인해주세요.', 'Please check your network connection.'],
  'auth_too_many': ['잠시 후 다시 시도해주세요.', 'Please try again later.'],
  'auth_generic': ['인증 오류가 발생했습니다.', 'An authentication error occurred.'],
  'auth_unknown': ['알 수 없는 오류가 발생했습니다.', 'An unknown error occurred.'],

  // 대시보드
  'greeting': ['안녕하세요, {name}님 👋', 'Hi, {name} 👋'],
  'streak_done': ['연속 출석 {n}회! 🎉', '{n} in a row! 🎉'],
  'streak_ongoing': ['연속 출석 {n}회차!', '{n} in a row!'],
  'streak_reward_ready': ['음료 리워드를 받을 수 있어요.', 'Your drink reward is ready.'],
  'streak_remaining': ['다음 리워드까지 {n}회 남았어요.', '{n} more session(s) until your next reward.'],
  'reward': ['리워드', 'Reward'],
  // 빽다방 리워드 섹션
  'rewards_appbar': ['리워드', 'Rewards'],
  'reward_section_title': ['빽다방 음료 쿠폰', 'Paik\'s Coffee coupon'],
  'reward_section_sub': [
    '하루 1개 발급 · 발급받은 쿠폰을 사용해야 다음 쿠폰을 받을 수 있어요 (부천역곡역북부점 사용)',
    'One coupon per day · use your coupon before claiming the next one.'
  ],
  'reward_claim_today': ['오늘의 음료 쿠폰 받기', "Claim today's drink coupon"],
  'reward_has_unused': [
    '사용하지 않은 쿠폰이 있어요. 사용 후 새 쿠폰을 받을 수 있어요.',
    'You have an unused coupon. Use it to claim a new one.'
  ],
  'reward_claimed_today': [
    '오늘은 이미 쿠폰을 발급받았어요. 내일 다시 받을 수 있어요.',
    "You've already claimed today's coupon. Come back tomorrow."
  ],
  'member_club': ['동아리원', 'Club member'],
  'member_general': ['일반학생', 'Student'],
  'member_club_desc': [
    '멘토링 그룹에 소속된 동아리원이에요.',
    "You're a club member in a mentoring group."
  ],
  'member_general_desc': [
    '동아리(멘토링 그룹)에 가입하면 동아리원이 돼요.',
    'Join a mentoring group to become a club member.'
  ],
  'qa_reward': ['리워드', 'Rewards'],
  'reward_stock': ['남은 수량', 'Remaining'],
  'my_coupons': ['내 쿠폰', 'My coupons'],
  'used_coupons': ['사용한 쿠폰', 'Used coupons'],
  'coupon_issued': ['쿠폰이 발급됐어요! 내 쿠폰에서 확인하세요.', 'Coupon issued!'],
  'coupon_sold_out': ['해당 음료 재고가 소진됐어요.', 'That drink is sold out.'],
  'coupon_not_eligible': ['아직 받을 수 있는 리워드가 없어요.', 'No reward available yet.'],
  'coupon_failed': ['쿠폰 발급에 실패했어요.', 'Failed to issue coupon.'],
  'choose_drink': ['음료를 선택하세요', 'Choose a drink'],
  'choose_drink_sub': ['쿠폰으로 받을 빽다방 음료 1종을 골라주세요.', 'Pick one drink to receive.'],
  'sold_out': ['품절', 'Sold out'],
  'remaining_n': ['{n}개 남음', '{n} left'],
  'reward_store': ['빽다방 부천역곡역북부점 · 매장 사용', 'Paik\'s Coffee Yeokgok N. · in-store only'],
  'issued_at': ['발급', 'Issued'],
  'used_at': ['사용', 'Used'],
  'coupon_used_badge': ['사용완료', 'Used'],
  'use_coupon': ['사용하기', 'Use'],
  'redeem_title': ['직원 확인', 'Staff verification'],
  'redeem_desc': [
    '매장 직원이 사용완료 코드(4자리)를 입력하면 쿠폰이 사용 처리됩니다.',
    'Store staff enters the 4-digit code to redeem this coupon.'
  ],
  'redeem_confirm': ['사용 처리', 'Redeem'],
  'redeem_success': ['쿠폰이 사용 처리됐어요.', 'Coupon redeemed.'],
  'redeem_bad_code': ['코드가 올바르지 않아요.', 'Incorrect code.'],
  'reward_ready_title': ['리워드를 받을 수 있어요! 🎉', 'Reward available! 🎉'],
  'reward_ready_body': [
    '{n}일 연속 출석 달성! 아래 리워드에서 음료 쿠폰을 받아보세요.',
    "{n}-day streak! Claim your drink coupon in the reward section below."
  ],
  'quick_actions': ['바로가기', 'Shortcuts'],
  'calendar_title': ['통합 일정', 'Calendar'],
  'open_calendar': ['통합 일정 보기', 'Open calendar'],
  'cal_attendance': ['출석일', 'Session'],
  'cal_attended': ['출석 완료', 'Attended'],
  'cal_deadline': ['{title} 마감', '{title} deadline'],
  'cal_no_events': ['이 날은 일정이 없습니다.', 'No events on this day.'],
  'cal_month_no_events': ['이번 달 일정이 없습니다.', 'No events this month.'],
  'cal_show_month': ['이번 달 전체 보기', 'Show whole month'],
  'cal_month_events': ['이번 달 일정', 'This month'],
  'cal_all_events': ['전체 일정', 'All schedule'],
  'legend_session': ['출석일', 'Session'],
  'legend_deadline': ['대외활동 마감', 'Activity deadline'],
  'summary_next_attend': ['다음 출석일', 'Next session'],
  'summary_drafts': ['미제출 보고서', 'Draft reports'],
  'summary_pending_rsvp': ['미응답 일정', 'Pending RSVP'],
  'summary_no_group': ['그룹 미참여', 'No group'],
  'dday_n': ['D-{n}', 'D-{n}'],
  'dday_today2': ['오늘', 'Today'],
  'count_n': ['{n}건', '{n}'],
  'qa_write_report': ['보고서 작성', 'Write report'],
  'qa_activities': ['대외활동', 'Activities'],
  'qa_checkin': ['출석체크', 'Check in'],
  'closing_soon_section': ['마감 임박 대외활동', 'Closing soon'],
  'see_all': ['전체보기', 'See all'],
  'no_upcoming': ['마감 예정 활동이 없습니다.', 'No upcoming deadlines.'],
  'links_section': ['홈페이지 · SNS', 'Homepage & SNS'],
  'univ_home': ['유한대학교 홈페이지', 'Yuhan University'],
  'dept_home': ['식품영양학과 홈페이지', 'Food & Nutrition Dept.'],
  'dept_insta': ['식품영양학과 인스타그램', 'Food & Nutrition Instagram'],
  'account': ['계정', 'Account'],
  'logout': ['로그아웃', 'Log out'],
  // 알림 설정
  'notif_settings': ['알림 설정', 'Notifications'],
  'notif_master': ['알림 받기', 'Enable notifications'],
  'notif_master_sub': ['끄면 아래 모든 알림이 비활성화됩니다.',
      'Turning off disables all alerts below.'],
  'notif_mentee_section': ['멘티 알림', 'Mentee alerts'],
  'notif_mentor_section': ['멘토·관리자 알림', 'Mentor/Admin alerts'],
  'notif_attend_reminder': ['출석일 리마인더', 'Attendance day reminder'],
  'notif_attend_reminder_sub': ['출석일 당일 아침에 알려줍니다.',
      'Reminds you on the morning of a session.'],
  'notif_rsvp_reminder': ['참석 응답 리마인더', 'RSVP reminder'],
  'notif_rsvp_reminder_sub': ['아직 응답 안 한 일정을 알려줍니다.',
      'Reminds you of unanswered sessions.'],
  'notif_schedule_added': ['새 일정 등록 알림', 'New schedule added'],
  'notif_schedule_added_sub': ['멘토가 출석일을 추가하면 알려줍니다.',
      'When your mentor adds a session date.'],
  'notif_new_notice': ['새 공지 알림', 'New notice'],
  'notif_new_report': ['새 보고서 제출 알림', 'New report submitted'],
  'notif_new_report_sub': ['멘티가 보고서를 제출하면 알려줍니다.',
      'When a mentee submits a report.'],
  'notif_rsvp_declined': ['불참 응답 알림', 'RSVP declined'],
  'notif_rsvp_declined_sub': ['멘티가 불참으로 응답하면 알려줍니다.',
      'When a mentee marks not-going.'],
  'notif_saved': ['알림 설정을 저장했습니다.', 'Notification settings saved.'],
  'notif_not_wired_note': [
    '※ 출석일·참석응답 리마인더는 기기에서 로컬 알림으로 발송됩니다. 새 보고서·공지 등 서버 알림은 추후 지원 예정입니다.',
    '※ Attendance & RSVP reminders are delivered as on-device local notifications. Server-side alerts (new reports/notices) are planned.'
  ],
  'change_photo': ['프로필 사진 변경', 'Change photo'],
  'remove_photo': ['프로필 사진 삭제', 'Remove photo'],
  'photo_updated': ['프로필 사진이 변경되었습니다.', 'Profile photo updated.'],
  'photo_failed': ['사진을 변경하지 못했습니다.', 'Failed to update photo.'],
  'dashboard_error': ['정보를 불러오지 못했습니다.\n아래로 당겨 새로고침 해주세요.',
      'Failed to load.\nPull down to refresh.'],
  'due': ['마감 {date}', 'Due {date}'],
  'closing_soon': ['마감임박', 'Closing soon'],
  // 언어 전환 메뉴: 현재 반대 언어명을 보여준다(ko일 때 English, en일 때 한국어).
  'switch_lang': ['English', '한국어'],
  'month_ko': ['{n}월', '{n}'],

  // 요일 (스트릭)
  'wd_mon': ['월', 'Mon'],
  'wd_tue': ['화', 'Tue'],
  'wd_wed': ['수', 'Wed'],
  'wd_thu': ['목', 'Thu'],
  'wd_fri': ['금', 'Fri'],

  // 대외활동
  'activities_title': ['대외활동', 'Activities'],
  'filter_all': ['전체', 'All'],
  'filter_food': ['식품·영양', 'Food & Nutrition'],
  'filter_past': ['지난 활동', 'Past'],
  'load_failed': ['불러오지 못했습니다.', 'Failed to load.'],
  'refresh': ['새로고침', 'Refresh'],
  'refreshed': ['최신 정보로 새로고침했습니다.', 'Refreshed with the latest.'],
  'no_activities': ['등록된 활동이 없습니다.', 'No activities yet.'],
  'no_food_activities': ['식품·영양 관련 활동이 아직 없습니다.', 'No food & nutrition activities yet.'],
  'no_past_activities': ['지난 활동이 없습니다.', 'No past activities.'],
  'dday': ['D-{n}', 'D-{n}'],
  'dday_today': ['D-DAY', 'D-DAY'],
  'closed': ['마감', 'Closed'],

  // 활동 종류
  'atype_fair': ['박람회/전시', 'Fair/Expo'],
  'atype_contest': ['공모전', 'Contest'],
  'atype_intern': ['인턴/채용', 'Intern/Job'],
  'atype_program': ['대외활동', 'Program'],
  'atype_seminar': ['세미나/특강', 'Seminar'],

  // 활동 상세
  'detail_organizer': ['주최/주관', 'Organizer'],
  'detail_eventdate': ['행사일', 'Event date'],
  'detail_deadline': ['신청마감', 'Deadline'],
  'detail_location': ['장소', 'Location'],
  'detail_content': ['상세 내용', 'Details'],
  'detail_more_hint': ['전체 공고와 신청은 원문 페이지에서 확인하세요.',
      'See the full posting and apply on the original page.'],
  'detail_open_link': ['신청/상세 페이지 열기', 'Open posting page'],
  'cannot_open_link': ['링크를 열 수 없습니다.', 'Cannot open the link.'],

  // 자격증·면허 정보
  'certs_title': ['자격증·면허 정보', 'Certifications & Licenses'],
  'open_certs': ['자격증·면허 정보', 'Certifications & Licenses'],
  'certs_subtitle': ['식품·영양 관련 자격증과 면허를 한눈에', 'Food & nutrition credentials at a glance'],
  'certs_section_license': ['면허 (국시원)', 'Licenses (KUKSIWON)'],
  'certs_section_tech': ['국가기술자격 (큐넷)', 'National Technical Certs (Q-Net)'],
  'certs_section_private': ['민간자격', 'Private Certifications'],
  'certs_section_international': ['국제자격', 'International'],
  'cert_eligibility': ['응시자격', 'Eligibility'],
  'cert_subjects': ['시험과목', 'Exam subjects'],
  'cert_method': ['시험방법', 'Format'],
  'cert_pass': ['합격기준', 'Passing criteria'],
  'cert_schedule': ['시행시기', 'Schedule'],
  'cert_fee': ['응시료', 'Fee'],
  'cert_issuer': ['시행기관', 'Issuer'],
  'cert_official': ['공식 사이트에서 확인', 'Check official site'],
  'certs_source_note': [
    '※ 큐넷(q-net.or.kr)·국시원(kuksiwon.or.kr)·NSCA Korea(nscakorea.com)·ISSN(sportsnutritionsociety.org)·NASM(nasm.org) 공개 정보를 정리했습니다. 일정·응시료 등은 매년 바뀌니 공식 사이트에서 최종 확인하세요.',
    '※ Summarized from Q-Net, KUKSIWON, NSCA Korea, ISSN & NASM. Schedules/fees change yearly — confirm on the official sites.'
  ],

  // 보고서 목록
  'reports_title': ['동아리 보고서', 'Club Reports'],
  'admin_mode': ['관리자 모드 · 전체 보고서', 'Admin · All reports'],
  'author': ['작성자', 'Author'],
  'delete': ['삭제', 'Delete'],
  'cancel': ['취소', 'Cancel'],
  'delete_report_title': ['보고서 삭제', 'Delete report'],
  'delete_report_body': ['이 보고서를 삭제할까요? 되돌릴 수 없습니다.',
      'Delete this report? This cannot be undone.'],
  'report_deleted': ['보고서를 삭제했습니다.', 'Report deleted.'],
  'admin_nav_reports': ['보고서', 'Reports'],
  'admin_nav_attendance': ['출석', 'Attendance'],
  'admin_nav_manage': ['관리자', 'Admins'],
  'admin_nav_groups': ['그룹', 'Groups'],
  'admin_nav_reward': ['리워드', 'Reward'],
  // 관리자 리워드 화면
  'reward_admin_title': ['리워드 관리', 'Reward management'],
  'redeem_code_section': ['사용완료 코드 (4자리)', 'Redeem code (4-digit)'],
  'redeem_code_hint': [
    '빽다방 부천역곡역북부점 직원이 쿠폰 사용 시 입력하는 코드입니다.',
    'Code the store staff enters to redeem coupons.'
  ],
  'code_must_be_4': ['코드는 4자리 숫자여야 합니다.', 'Code must be 4 digits.'],
  'code_saved': ['코드를 저장했습니다.', 'Code saved.'],
  'reward_stock_section': ['음료 재고', 'Drink stock'],
  'stock_count': ['수량', 'Count'],
  'issued_coupons': ['발급 쿠폰', 'Issued coupons'],
  'coupon_stats': ['발급 {issued} · 사용 {used}', 'Issued {issued} · Used {used}'],
  'no_coupons': ['아직 발급된 쿠폰이 없습니다.', 'No coupons issued yet.'],
  'coupon_active_badge': ['사용가능', 'Active'],
  'groups_title': ['그룹 관리', 'Groups'],
  'create_group': ['그룹 만들기', 'Create group'],
  'group_name': ['그룹(멘토) 이름', 'Group (mentor) name'],
  'group_pin': ['4자리 PIN', '4-digit PIN'],
  'group_pin_hint': ['멘티가 이 PIN으로 그룹에 참여합니다.',
      'Mentees join with this PIN.'],
  'group_created': ['그룹을 만들었습니다.', 'Group created.'],
  'no_groups': ['등록된 그룹이 없습니다.', 'No groups yet.'],
  'group_members_n': ['멤버 {n}명', '{n} members'],
  'delete_group': ['그룹 삭제', 'Delete group'],
  'delete_group_body': ['이 그룹을 삭제할까요?', 'Delete this group?'],
  'group_deleted': ['그룹을 삭제했습니다.', 'Group deleted.'],
  'group_reports_title': ['{name} · 보고서', '{name} · Reports'],
  'err_pin4': ['숫자 4자리를 입력하세요.', 'Enter 4 digits.'],
  // 멘티 그룹 참여
  'select_mentor': ['멘토(그룹) 선택', 'Select mentor (group)'],
  'my_group': ['내 그룹', 'My group'],
  'join_group': ['그룹 참여', 'Join group'],
  'enter_pin': ['PIN 입력', 'Enter PIN'],
  'join_success': ['그룹에 참여했습니다.', 'Joined the group.'],
  'join_failed': ['PIN이 올바르지 않습니다.', 'Incorrect PIN.'],
  'report_group': ['그룹', 'Group'],
  'no_group_join_first': ['먼저 출석 탭에서 멘토를 선택하세요.', 'Choose a mentor in the Attendance tab first.'],
  // 멘토 선택(출석 화면)
  'choose_mentor': ['멘토를 선택하세요', 'Choose your mentor'],
  'choose_mentor_sub': [
    '멘토가 알려준 4자리 비밀번호를 입력하면 가입됩니다.',
    'Enter the 4-digit password your mentor gave you to join.'
  ],
  'no_mentors': ['등록된 멘토가 아직 없어요.', 'No mentors registered yet.'],
  'mentor_tap_join': ['눌러서 비밀번호로 가입', 'Tap to join with password'],
  'mentor_pin_desc': [
    '멘토가 알려준 4자리 비밀번호를 입력하세요.',
    'Enter the 4-digit password from your mentor.'
  ],
  'mentor_join': ['가입', 'Join'],
  'mentor_pin_4': ['비밀번호는 4자리입니다.', 'Password must be 4 digits.'],
  'mentor_joined': ['{name} 멘토에 가입했어요!', 'Joined mentor {name}!'],
  'mentor_pin_wrong': ['비밀번호가 올바르지 않아요.', 'Incorrect password.'],
  'change_mentor': ['멘토 변경', 'Change mentor'],
  'select_mentor_first': ['먼저 멘토를 선택하세요.', 'Please choose a mentor first.'],
  // 앱 초기화
  'reset_app': ['앱 초기화', 'Reset app'],
  'reset_app_warn': [
    '내 보고서·출석·리워드 쿠폰·멘토 가입·프로필/설정이 모두 삭제되고 로그아웃됩니다.\n이 작업은 되돌릴 수 없어요.',
    'Your reports, attendance, coupons, mentor membership, and profile/settings will be permanently deleted and you will be signed out. This cannot be undone.'
  ],
  'reset_confirm': ['초기화', 'Reset'],
  'reset_done': ['초기화됐어요.', 'Reset complete.'],
  'reset_failed': ['초기화에 실패했어요. 잠시 후 다시 시도해주세요.', 'Reset failed. Please try again.'],
  // 관리자: 회원 계정 초기화
  'reset_member': ['계정 초기화', 'Reset account'],
  'reset_member_title': ['회원 계정을 초기화할까요?', 'Reset this member?'],
  'reset_member_body': [
    '{name} 님의 보고서·출석·리워드 쿠폰·멘토 가입·프로필/설정이 모두 삭제됩니다.\n로그인 계정은 남아 있어 다시 이용할 수 있어요.\n이 작업은 되돌릴 수 없습니다.',
    'All reports, attendance, coupons, mentor membership, and profile/settings for {name} will be permanently deleted. The login account remains usable. This cannot be undone.'
  ],
  'reset_member_done': ['{name} 님의 계정을 초기화했어요.', 'Reset {name}\'s account.'],
  // 관리자: 멤버 계정 찾기/비밀번호 재설정
  'member_accounts': ['멤버 계정 관리', 'Member accounts'],
  'member_accounts_hint': [
    '멘티가 아이디(이메일)나 비밀번호를 잊었을 때 여기서 확인·재설정할 수 있어요.',
    "Look up a member's login ID or reset their password here."
  ],
  'login_id': ['아이디', 'Login ID'],
  'issue_temp_pw': ['임시 비밀번호 발급', 'Issue temp password'],
  'reset_pw_title': ['비밀번호 재설정', 'Reset password'],
  'reset_pw_body': [
    '{name} 님의 임시 비밀번호를 새로 발급할까요? 기존 비밀번호는 사용할 수 없게 됩니다.',
    'Issue a new temporary password for {name}? The old password will stop working.'
  ],
  'reset_pw_confirm': ['발급', 'Issue'],
  'reset_pw_failed': ['비밀번호 재설정에 실패했어요.', 'Failed to reset password.'],
  'temp_pw_title': ['임시 비밀번호가 발급됐어요', 'Temporary password issued'],
  'temp_pw_desc': [
    '아래 정보를 멤버에게 전달하세요. 로그인 후 비밀번호를 바꾸도록 안내해 주세요.',
    'Share this with the member. Ask them to change it after logging in.'
  ],
  'temp_pw': ['임시 비밀번호', 'Temp password'],
  'copied': ['복사됐어요.', 'Copied.'],
  'select_group': ['그룹 선택', 'Select group'],
  'admin_manage_title': ['관리자 관리', 'Admin management'],
  'add_admin_section': ['새 관리자 추가', 'Add new admin'],
  'add_admin_hint': ['계정이 Authentication에 자동 등록됩니다.',
      'The account is auto-registered in Authentication.'],
  'add_admin_button': ['관리자 추가', 'Add admin'],
  'admin_added': ['관리자를 추가했습니다.', 'Admin added.'],
  'add_admin_failed': ['관리자 추가 실패', 'Failed to add admin'],
  'current_admins': ['현재 관리자', 'Current admins'],
  'remove_admin_title': ['관리자 해제', 'Remove admin'],
  'remove_admin_body': ['이 관리자의 권한을 해제할까요?', 'Remove this admin?'],
  'admin_removed': ['관리자를 해제했습니다.', 'Admin removed.'],
  'you_label': ['(나)', '(you)'],
  'admin_attendance_title': ['출석 확인', 'Attendance'],
  'admin_attendance_summary': ['총 {n}명 출석 기록', '{n} members with records'],
  'attendance_none': ['출석 기록이 없습니다.', 'No attendance records.'],
  'checked_today_yes': ['오늘 출석', 'Checked in today'],
  'checked_today_no': ['오늘 미출석', 'Not checked in'],
  'reward_reached_badge': ['🎁 보상 대상', '🎁 Reward due'],
  'streak_unit': ['연속 {n}회', '{n} in a row'],
  'total_unit': ['누적 {n}일', '{n} total'],
  'report_detail_title': ['보고서 상세', 'Report detail'],
  'report_role': ['역할', 'Role'],
  'report_partner': ['상대방', 'Partner'],
  'report_activity_date': ['활동 일자', 'Activity date'],
  'report_no_content': ['작성된 내용이 없습니다.', 'No content.'],
  'rf_all': ['전체', 'All'],
  'rf_draft': ['임시저장', 'Draft'],
  'rf_submitted': ['제출완료', 'Submitted'],
  'rf_mentor': ['멘토', 'Mentor'],
  'rf_mentee': ['멘티', 'Mentee'],
  'reports_empty': ['작성한 보고서가 없습니다.', 'No reports yet.'],
  'reports_load_failed': ['보고서를 불러오지 못했습니다.', 'Failed to load reports.'],
  'reports_empty_hint': ['우측 하단 버튼으로 첫 보고서를 작성해보세요.',
      'Tap the button at the bottom right to write your first report.'],
  'hours': ['{n}시간', '{n}h'],
  'new_report': ['새 보고서 작성', 'New report'],
  'role_mentor': ['멘토', 'Mentor'],
  'role_mentee': ['멘티', 'Mentee'],
  'status_draft': ['임시저장', 'Draft'],
  'status_submitted': ['제출완료', 'Submitted'],

  // 보고서 편집
  'report_new_title': ['새 보고서 작성', 'New Report'],
  'report_edit_title': ['보고서 수정', 'Edit Report'],
  'report_name_label': ['이름', 'Name'],
  'report_name_hint': ['본인 이름을 입력하세요', 'Enter your name'],
  'required_tag': ['필수', 'Required'],
  'report_title_hint': ['예: 5월 정기모임 및 조리실습', 'e.g. May regular meeting & cooking practice'],
  'report_content_hint': [
    '오늘 진행한 동아리 활동 내용을 자세히 적어주세요.\n(어떤 활동을 했는지, 무엇을 배우고 느꼈는지 등)',
    'Describe today\'s club activity in detail.\n(what you did, what you learned, etc.)'
  ],
  'report_feedback_hint': [
    '활동을 통해 느낀 점이나 서로에게 남기고 싶은 피드백을 적어주세요.',
    'Share your reflection or feedback from the activity.'
  ],
  'hours_n_label': ['{n}시간', '{n}h'],
  'report_partner_required': ['함께한 멘티/멘토 이름을 입력해주세요.', 'Please enter the mentee/mentor name.'],
  'report_author_role': ['나의 역할', 'Author role'],
  'select_role_first': ['작성자 역할을 선택하세요.', 'Please select the author role.'],
  'report_title_field': ['제목', 'Title'],
  'partner_mentee': ['멘티 이름', 'Mentee name'],
  'partner_mentor': ['멘토 이름', 'Mentor name'],
  'report_date': ['활동 날짜', 'Activity date'],
  'report_time': ['활동 시간', 'Activity time'],
  'report_hours': ['활동 소요시간 (시간)', 'Duration (hours)'],
  'report_content': ['활동 내용', 'Activity details'],
  'report_feedback_field': ['소감/피드백', 'Reflection/Feedback'],
  'report_photos': ['사진 (최대 4장)', 'Photos (up to 4)'],
  'add_photo': ['사진 추가', 'Add photo'],
  'photo_pick_failed': ['사진을 추가하지 못했습니다.', 'Failed to add photo.'],
  'field_required': ['{label}을(를) 입력해주세요', 'Please enter {label}'],
  'save_draft': ['임시저장', 'Save draft'],
  'submit': ['제출', 'Submit'],
  'saved_draft_msg': ['임시저장했습니다.', 'Saved as draft.'],
  'submitted_msg': ['보고서를 제출했습니다.', 'Report submitted.'],
  'save_failed': ['저장 실패: {e}', 'Save failed: {e}'],

  // 출석
  'attendance_title': ['출석', 'Attendance'],
  'attendance_streak_label': ['연속 출석 스트릭', 'Attendance streak'],
  'streak_days': ['{n}일', '{n} day(s)'],
  'streak_encourage': ['꾸준함이 쌓이고 있어요.', 'Your consistency is building up.'],
  'attend_reward': ['출석 리워드', 'Attendance reward'],
  'coffee_gifticon': ['빽다방 음료 쿠폰', 'Paik\'s Coffee drink coupon'],
  'reward_condition': ['{n}일 연속 출석 시 지급', 'Given after a {n}-day streak'],
  'check_in_today': ['오늘 출석하기', 'Check in today'],
  'not_attendance_day': ['오늘은 출석일이 아닙니다', 'Not an attendance day'],
  'next_attendance_date': ['다음 출석일: {date}', 'Next: {date}'],
  'no_scheduled_dates': ['등록된 출석일이 없습니다.', 'No scheduled dates.'],
  'attendance_schedule': ['출석 일정', 'Attendance schedule'],
  'scheduled_day': ['예정', 'Scheduled'],
  'done_mark': ['출석', 'Done'],
  'rsvp_available': ['참석', 'Going'],
  'rsvp_unavailable': ['불참', 'Not going'],
  'rsvp_reason': ['불참 사유', 'Reason'],
  'rsvp_reason_hint': ['불참 사유를 입력하세요(선택)', 'Reason (optional)'],
  'rsvp_saved': ['참석 여부를 저장했습니다.', 'RSVP saved.'],
  'rsvp_title': ['{date} 참석하시나요?', 'Attending on {date}?'],
  'rsvp_none': ['미응답', 'No response'],
  'save': ['저장', 'Save'],
  'group_rsvp': ['참석 응답 현황', 'RSVP status'],
  'admin_nav_stats': ['통계', 'Stats'],
  'stat_members': ['회원 수', 'Members'],
  'stat_sessions': ['출석일 수', 'Sessions'],
  'stat_attendance_rate': ['평균 출석률', 'Attendance rate'],
  'stat_reports_total': ['총 보고서', 'Total reports'],
  'stat_submitted': ['제출완료', 'Submitted'],
  'stat_draft': ['임시저장', 'Draft'],
  'stat_rsvp_rate': ['응답률', 'RSVP rate'],
  'export_csv': ['CSV 복사(내보내기)', 'Copy CSV'],
  'csv_copied': ['CSV를 클립보드에 복사했습니다.', 'CSV copied to clipboard.'],
  'stat_per_member': ['회원별', 'Per member'],
  'col_name': ['이름', 'Name'],
  'col_attend': ['출석', 'Attend'],
  'col_reports': ['보고서', 'Reports'],
  'add_attendance_date': ['날짜 추가', 'Add date'],
  'schedule_hint': ['등록한 날짜에만 회원이 출석할 수 있습니다.',
      'Members can only check in on scheduled dates.'],
  'date_added': ['출석일을 추가했습니다.', 'Date added.'],
  'confirm': ['확인', 'Confirm'],
  'today': ['오늘', 'Today'],
  'tomorrow': ['내일', 'Tomorrow'],
  'next_session': ['다음 일정', 'Next session'],
  'schedule_topic': ['일정 주제', 'Topic'],
  'schedule_topic_hint': ['예: OT, 조별과제, 특강 등', 'e.g. Orientation, Team project'],
  'schedule_no_topic': ['주제 미정', 'No topic'],
  'date_removed': ['출석일을 삭제했습니다.', 'Date removed.'],
  'already_checked': ['오늘은 이미 출석했습니다.', "You've already checked in today."],
  'attendance_error': ['출석 정보를 불러오지 못했습니다.', 'Failed to load attendance.'],
  'attendance_appbar': ['출석 체크', 'Check In'],
  'checkin_success': ['출석 완료! 연속 {n}회째 🎉', 'Checked in! {n} in a row 🎉'],
  'coffee_achieved_title': ['리워드 달성!', 'Reward unlocked!'],
  'coffee_achieved_body': [
    '{n}일 연속 출석을 달성했어요!\n출석 화면 리워드에서 음료 쿠폰을 받아보세요.',
    "You've reached a {n}-day streak!\nClaim your drink coupon in the reward section."
  ],
  'ok_great': ['좋아요!', 'Great!'],
  'streak_now': ['현재 {n}회 연속 출석 중!', "You're on a {n}-session streak!"],
  'streak_start': ['오늘 출석을 시작해보세요', 'Start your streak today'],
  'total_days': ['누적 출석 {n}회', '{n} sessions total'],
  'day_n': ['{n}회', '#{n}'],
  'attended_day': ['출석한 날', 'Attended'],
  'coffee_goal': ['음료 리워드 목표', 'Drink reward goal'],
  'count_of': ['{c}/{t}회', '{c}/{t}'],
  'coffee_ready': ['🎉 아래 리워드에서 음료 쿠폰을 받으세요!',
      '🎉 Claim your drink coupon in the reward section below!'],
  'coffee_hint': ['출석 {goal}회마다 음료 쿠폰 지급 · 다음 쿠폰까지 {n}회 남음',
      'A drink coupon every {goal} sessions · {n} left to next'],
  'checked_done': ['오늘 출석 완료  ☕', "Checked in today  ☕"],
  'attendance_note': [
    '연속 출석이 끊기면 다시 1회부터 시작됩니다. 리워드 음료 쿠폰은 빽다방 부천역곡역북부점에서 사용할 수 있습니다.',
    'If your streak breaks, it restarts from day 1. Drink coupons can be used at the Paik\'s Coffee Yeokgok North branch.'
  ],
};

/// 현재 로케일에 맞는 문자열. `{key}` 플레이스홀더를 args 로 치환.
String tr(BuildContext context, String key, [Map<String, String>? args]) {
  final code = Localizations.localeOf(context).languageCode;
  final idx = code == 'en' ? 1 : 0;
  final entry = _dict[key];
  var out = entry == null ? key : entry[idx];
  if (args != null) {
    args.forEach((k, v) => out = out.replaceAll('{$k}', v));
  }
  return out;
}
